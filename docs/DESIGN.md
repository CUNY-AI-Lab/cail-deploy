...
OAuth metadata sends browser authorization to Doorway's unlisted `/api/oauth/authorize` route. Doorway performs CUNY login and privately forwards the exact consent request with a short-lived `cail:deploy` identity. Deploy still parses and completes the authorization, stores the consent nonce, issues the code and access token, and owns the MCP resource; Doorway never receives the OAuth access token.
