...
The digest check prevents a producer or transport error from substituting different bytes for the requested revision. The owner comparison is required because the HTTP and MCP surfaces are multi-tenant. Prepared activation and retry follow the ambiguous-final-write failure already reproduced in CAIL-deploy at `1fd95315b2d4f3e5573383030856a7844ffa4151`.

## Scope decisions

GitHub, repository ownership, webhooks, check runs, installation tokens, AWS dispatch, and old identifiers are absent. Linux/Sandbox builds are deferred until a first Linux-only artifact enters the integration scenario. Project D1/R2/KV provisioning and project secrets are deferred; this slice rejects all requested bindings, which keeps publication isolated without inventing a resource policy engine. The isolated deployment verifies an offline run-scoped CAIL identity issuer with the accepted identity 4.6.0 source package. The local bearer map exists only for component tests.

## Recovery and rollback

D1 is authoritative and R2 holds replay bytes. The isolated stack can be exported with `wrangler d1 export` and `wrangler r2 object get` for named keys. No SDK-specific durable state is stored. Safe teardown uses only exact names and provider IDs in `resources/manifest.json`.
