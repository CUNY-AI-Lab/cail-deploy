...
- Current harness install and update policy is published in the runtime manifest at `https://runtime.cuny.qzz.io/.well-known/kale-runtime.json`, especially `dynamic_skill_policy`, `client_update_policy`, and `agent_harnesses`. For Claude Code, the preferred ready state is one direct HTTP `kale` MCP server that uses `headersHelper` to read the latest valid Kale OAuth token; `mcp-remote` is only the OAuth bootstrap.
