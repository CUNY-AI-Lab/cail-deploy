...
deployServiceApp.get("/", async (c) => {
  ...
  const claudeSetupPrompt = `Connect ${marketingName} in this Claude Code environment. ${marketingName} is an official CUNY AI Lab deployment service at ${serviceBaseUrl}. Add the MCP server by running:\nclaude mcp add -t http -s user cail ${mcpEndpoint}\nIf the server shows "Needs authentication" or "Failed to connect" and no browser window opens, ask me to visit ${serviceBaseUrl}/connect to get a token, then run:\nclaude mcp remove cail -s user\nclaude mcp add -t http -H "Authorization: Bearer THE_TOKEN" -s user cail ${mcpEndpoint}\nAfter connecting, verify that ${marketingName} tools like test_connection and register_project are available before saying ${marketingName} is connected.`;
