...
test("project control panel can change the primary Kale URL and manage redirect labels", async (t) => {
  const { env, db } = createTestContext({
    CLOUDFLARE_ACCESS_TEAM_DOMAIN: "https://access.example",
    CLOUDFLARE_ACCESS_AUD: "test-access-aud",
    CLOUDFLARE_ACCESS_ALLOWED_EMAIL_DOMAINS: "cuny.edu"
  ...
  assert.match(html, /https:\/\/kale-cache-smoke-test\.cuny\.qzz\.io/);
  ...
  primaryForm.set("formToken", formToken);
  primaryForm.set("domainLabel", "cache-smoke");
  const primaryResponse = await fetchRaw(new Request("https://deploy.example/projects/kale-cache-smoke-test/control/domains/primary", {
    method: "POST",
    headers: {
      "cf-access-jwt-assertion": accessJwt,
      "cf-access-authenticated-user-email": "person@cuny.edu"
  ...
  assert.equal(primaryResponse.status, 302);
  assert.match(primaryResponse.headers.get("location") ?? "", /flash=success/);
  assert.equal(db.selectPrimaryProjectDomain("kale-cache-smoke-test")?.domain_label, "cache-smoke");
  assert.equal(db.selectProjectDomain("kale-cache-smoke-test")?.is_primary, 0);
  assert.equal(
    db.selectProject("kale-cache-smoke-test")?.deployment_url,
    "https://cache-smoke.cuny.qzz.io"
  );

  const redirectForm = new FormData();
  redirectForm.set("formToken", formToken);
  redirectForm.set("domainLabel", "cache-preview");
  const redirectResponse = await fetchRaw(new Request("https://deploy.example/projects/kale-cache-smoke-test/control/domains/redirects", {
    method: "POST",
    headers: {
      "cf-access-jwt-assertion": accessJwt,
      "cf-access-authenticated-user-email": "person@cuny.edu"
  ...
  assert.ok(db.selectProjectDomain("cache-preview"));

  const deleteForm = new FormData();
  deleteForm.set("formToken", formToken);
  const deleteResponse = await fetchRaw(new Request("https://deploy.example/projects/kale-cache-smoke-test/control/domains/cache-preview/delete", {
    method: "POST",
    headers: {
      "cf-access-jwt-assertion": accessJwt,
      "cf-access-authenticated-user-email": "person@cuny.edu"
  ...
  assert.equal(db.selectProjectDomain("cache-preview"), null);
});
