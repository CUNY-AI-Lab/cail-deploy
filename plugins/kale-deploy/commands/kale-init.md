...
2. Create a Kale Deploy starter directly in this directory. Do not clone or fetch a template repo.
3. Write:
   - `package.json`
   - `wrangler.jsonc`
   - `tsconfig.json`
...
6. Define standard Kale binding names in code: `DB`, `FILES`, `CACHE`, `AI`, `VECTORIZE`, and `ROOMS`.
...
9. Explain that production bindings are attached by the Kale deploy service, even if local `wrangler.jsonc` only contains placeholders.
