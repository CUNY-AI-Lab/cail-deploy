...
7. After OAuth succeeds in Claude Code, immediately replace the temporary bridge with one direct HTTP `kale` server:
...
8. Do not invent `claude mcp auth`, `claude mcp login`, or `claude mcp authenticate` commands. Only if `mcp-remote` fails, ask the user to open `https://cuny.qzz.io/kale/connect`, sign in with a CUNY email, generate a token, and paste it back.
