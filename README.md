...
- Claude Code should use `mcp-remote` only to complete OAuth, then finalize to one direct HTTP `kale` server with the installed `kale-claude-connect.mjs` helper; the `/connect` token bridge is the last resort.
